"""
Reusable evaluation helpers, adapted from the course to the Anthropic SDK.

The course uses OpenAI's `responses.parse(text_format=PydanticModel)` for
structured output. The Anthropic Messages API has no `.parse()`, so we get the
same guarantee a different way: we hand the model a single tool whose
`input_schema` IS our pydantic schema, and force it to call that tool
(`tool_choice`). The tool-call arguments then come back as a validated dict.

This file contains:
    - llm_structured       structured LLM call -> pydantic object + usage
    - llm_structured_retry same, with exponential-backoff retries
    - calc_price / calc_total_price  cost from token usage
    - RAGWithUsage         RAGBase subclass that records per-call usage
    - map_progress         run a function over a list in parallel w/ a tqdm bar
"""
